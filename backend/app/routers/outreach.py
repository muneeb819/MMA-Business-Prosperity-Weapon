...
@router.get("/cadence")
def get_cadence(user=Depends(get_current_user)):
    return {"cadence": outreach_service.CADENCE}


@router.get("/leads")
def outreach_leads(db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def enrich_lead(lead_id: str, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def enrich_all(db: Session = Depends(get_db), user=Depends(get_current_user)):
...
@router.post("/preview")
def preview(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
@router.post("/send")
def send(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    lead_id = data.get("lead_id")
    step = int(data.get("step", 0))
    lead = db.query(Lead).filter(Lead.id == lead_id).first()
    if not lead:
...
def automation_run(data: dict = None, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def automation_status(db: Session = Depends(get_db), user=Depends(get_current_user)):
    return outreach_automation.status(db)


@router.post("/automation/enroll")
def automation_enroll(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return outreach_automation.enroll(db, data.get("lead_id"))


@router.post("/automation/pause")
def automation_pause(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return outreach_automation.set_paused(db, data.get("lead_id"), True)


@router.post("/automation/resume")
def automation_resume(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return outreach_automation.set_paused(db, data.get("lead_id"), False)


@router.post("/automation/settings")
def automation_settings(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def wework_agent_run(data: dict = None, db: Session = Depends(get_db), user=Depends(get_current_user)):
    import asyncio
...
def wework_agent_enrich(data: dict = None, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def wework_agent_status(db: Session = Depends(get_db), user=Depends(get_current_user)):
    wwr_leads = (
        db.query(Lead)
        .filter(Lead.platform == "weworkremotely")
        .order_by(Lead.found_at.desc())
...
@router.get("/records")
def records(db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def mark_reply(outreach_id: str, db: Session = Depends(get_db), user=Depends(get_current_user)):
