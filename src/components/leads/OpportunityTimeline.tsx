...
const stageConfig: Record<string, { icon: typeof Search; color: string }> = {
  discovered: { icon: Search, color: "text-indigo-400" },
  analyzing: { icon: Brain, color: "text-rose-400" },
  proposal: { icon: FileText, color: "text-indigo-400" },
  submitted: { icon: Send, color: "text-emerald-400" },
  interview: { icon: MessageSquare, color: "text-amber-400" },
  negotiation: { icon: Handshake, color: "text-rose-400" },
          ...
          <div key={stage.stage} className="flex gap-3">
            <div className="flex flex-col items-center">
              <div className={cn(
                ...
                isPast ? "bg-emerald-500/10 border-emerald-500/40" : isActive ? "bg-indigo-500/10 border-indigo-400" : "bg-zinc-800/50 border-zinc-700/50"
              )}>
                {isPast ? (
                  <div className="w-2.5 h-2.5 rounded-full bg-emerald-400" />
                ) : (
                  <Icon className={cn("w-4 h-4", isActive ? "text-indigo-400" : "text-zinc-600")} />
                )}
              </div>
            ...
            <div className={cn("pb-6 min-w-0 flex-1", isLast && "pb-0")}>
              <div className="flex items-center gap-2">
                <span className={cn(
                  "text-sm font-medium",
                  isPast ? "text-emerald-300" : isActive ? "text-indigo-300" : "text-zinc-500"
                )}>
                  {stage.label}
                </span>
                {stage.date && (
                ...
                {isActive && (
                  <Badge variant="outline" className="text-[9px] bg-indigo-500/10 text-indigo-400 border-indigo-500/20 px-1.5 py-0">
                    Current
                  </Badge>
                )}
              </div>
