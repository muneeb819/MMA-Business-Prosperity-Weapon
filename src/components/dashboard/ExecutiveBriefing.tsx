...
const ExecutiveBriefing = memo(function ExecutiveBriefing({ briefing, loading, onViewAll }: ExecutiveBriefingProps) {
  ...
  return (
    <Card className="bg-gradient-to-br from-zinc-900 to-zinc-900/80 border-indigo-500/20 overflow-hidden relative">
      <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/5 rounded-full blur-3xl pointer-events-none" />
      <CardContent className="p-5 relative">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500/20 to-rose-500/20 border border-indigo-500/20 flex items-center justify-center">
          ...
          <Button variant="ghost" size="sm" onClick={onViewAll} className="text-xs text-indigo-400 hover:text-indigo-300 h-8">
        ...
        <div className="flex items-center gap-4 mb-4 flex-wrap">
          <Badge variant="outline" className="bg-indigo-500/10 text-indigo-400 border-indigo-500/20 text-xs font-medium px-3 py-1">
          ...
          <div className="space-y-2">
            ...
            {briefing.top_recommendations.map((rec, i) => (
              <div key={i} className={cn("flex items-start gap-3 p-3 rounded-xl bg-zinc-800/30 border border-zinc-800/50")}>
                <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-indigo-500 to-rose-600 flex items-center justify-center shrink-0 text-xs font-bold text-white">
                  {i + 1}
                </div>
