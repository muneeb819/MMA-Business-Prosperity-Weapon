...
| Stage | Description |
|-------|-------------|
| **Hunting** | Backend fetches real leads from 6+ live job boards (Himalayas, RemoteOK, Remotive, We Work Remotely, Arbeitnow, Findwork) via `backend/app/services/sync.py`, normalized and persisted into the `Lead` table. The `opportunity-hunter` page also has a client-side fetch path that caches results in localStorage for fast reloads, but the source of truth is the backend database. |
| **Landing** | Leads persisted server-side (SQLite in dev, Postgres in the docker-compose stack); the frontend caches a copy in localStorage for offline/optimistic UI, not as the primary store. |
| **Outreach** | AI generates tailored proposals using OpenAI (cover letter, technical plan, cost estimate, call to action); the ACIE pipeline (`backend/app/services/acie/`) handles contact scoring, compliance gating (do-not-contact/frequency caps), and cadence scheduling for real outreach sends. |
| **Response** | Submit proposals, send emails via SMTP (falls back to mailto: links), track status through pipeline |

> **Note on the "AI Teams" dashboard:** the 15 named agents shown on `/ai-teams` (Scout Alpha–Foxtrot, Contact Alpha–Foxtrot, Director AI, Quality Sentinel) currently render simulated task counts and metrics from a deterministic seeded random generator for demo purposes — they do not yet execute real hunting/outreach work themselves. The actual automated work is done by the 3 agents in `backend/app/routers/agents.py` (Opportunity Hunter, Lead Analyzer, Proposal Generator) and the ACIE outreach pipeline.
